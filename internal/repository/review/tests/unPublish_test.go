...
func TestUnPublish(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			reviewRepositoryMockFunc: func(mc *gomock.Controller) repository.ReviewRepository {
				...
				mock.EXPECT().UnPublish(ctx, id).Return(id, nil)
				return mock
			},
		},
		{
			...
			reviewRepositoryMockFunc: func(mc *gomock.Controller) repository.ReviewRepository {
				mock := repoMocks.NewMockReviewRepository(mc)
				mock.EXPECT().UnPublish(ctx, id).Return(0, repoErr)
				return mock
			},
		},
	}
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			result, err := reviewRepositoryMock.UnPublish(test.input.ctx, test.input.id)

			const caseOk = "OK (UnPublish)"
